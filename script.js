...
(function () {
  ...
  var FOOTER_HEADING = bi(
    "Heeyeon Jang, a service planner who improves service quality through careful review and testing.",
    "꼼꼼한 검수와 테스트로 서비스 완성도를 높이는 기획자 장희연입니다."
  );
  var TAGLINE = bi(
    "A service planner who turns requirements into planning documents, collaborates with developers and designers, and improves service quality through careful review and testing.",
    "요구사항을 기획 문서로 구체화하고, 개발자·디자이너와 협업하며 꼼꼼한 검수와 테스트로 서비스 완성도를 높이는 기획자입니다."
  );
  var ABOUT_H2 = bi(
    "A Planner Who Never Misses the Small Differences",
    "작은 차이를 놓치지 않는 기획자"
  );
  var ABOUT_BODY = bi(
    "After majoring in design, I found service planning compelling and started my career as a planner. Over about 2 years and 4 months, I managed shopping mall service operations and feature improvements — turning requirements into planning documents and running the service in close collaboration with developers and designers. By carefully reviewing image, policy, and data errors and testing under a range of conditions, I learned that service quality is built through checking even the smallest details.",
    "디자인을 전공한 후 서비스 기획의 매력을 느껴 기획자로 커리어를 시작했습니다. 약 2년 4개월 동안 쇼핑몰 서비스 운영과 기능 개선을 담당하며 요구사항을 기획 문서로 구체화하고, 개발자·디자이너와 협업하여 서비스를 운영해 왔습니다. 이미지·정책·데이터 오류를 꼼꼼히 검수하고, 다양한 조건에서 테스트를 진행하며 서비스 품질은 작은 부분까지 확인하는 과정에서 만들어진다는 것을 배웠습니다."
  );

  var ABOUT_CARDS = [
    { no: "01", title: bi("I notice even the smallest differences", "작은 차이까지 세심하게 살핍니다"),
      body: bi("Drawing on my background in visual design and service planning, I pay close attention to the smallest details. By verifying screens and features under a range of conditions, I learned that small differences can change a service's polish and user experience.",
        "시각디자인과 서비스 기획 경험을 바탕으로 작은 디테일까지 세심하게 살핍니다. 화면과 기능을 다양한 조건에서 검증하며, 작은 차이가 서비스의 완성도와 사용자 경험을 바꿀 수 있다는 것을 배웠습니다.") },
    { no: "02", title: bi("I grow by learning what I need", "필요한 것은 직접 배우며 성장합니다"),
      body: bi("To collaborate smoothly with developers, I taught myself SQL and data structures and applied them to my work. I keep actively learning whatever knowledge a new field requires, expanding the scope of what I can do.",
        "개발자와의 원활한 협업을 위해 SQL과 데이터 구조를 스스로 공부하고 업무에 적용했습니다. 새로운 분야에서도 필요한 지식을 적극적으로 배우며 업무의 범위를 넓혀가고 있습니다.") },
    { no: "03", title: bi("I structure complex work", "복잡한 업무를 구조화합니다"),
      body: bi("When multiple requests were running at once, I improved how I managed them so that tasks, priorities, and release schedules could all be checked at a glance. I organize and structure complex information to reduce the gaps and confusion that can arise during collaboration.",
        "여러 요청이 동시에 진행되는 상황에서 업무 내용과 우선순위, 배포 일정을 한눈에 확인할 수 있도록 관리 방식을 개선했습니다. 복잡한 정보를 정리하고 구조화하여 협업 과정에서 발생할 수 있는 누락과 혼선을 줄입니다.") },
    { no: "04", title: bi("I improve by reading the data", "데이터를 보고 개선을 반복합니다"),
      body: bi("Selling digital products I designed myself on Etsy, I analyzed search and sales data to continuously improve my designs and listings. By repeatedly feeding user response back into the next design, I achieved over 310 sales.",
        "Etsy에서 직접 디자인한 디지털 상품을 판매하며 검색 및 판매 데이터를 분석하고 디자인과 리스팅을 지속적으로 개선했습니다. 사용자 반응을 다음 디자인에 반영하는 과정을 반복하여 310건 이상의 판매를 달성했습니다.") }
  ];
  var WORKING_STYLE = [
  ...
  function renderHome() {
    ...
    return (
      ...
      '<div class="badge">Service Planner</div>' +
